use fltk::{prelude::*, *, enums::*, button::Button, app::Scheme, menu::MenuFlag};
use crate::flencoder_ui::UserInterface;
use fltk_theme::{WidgetTheme, ThemeType};
use fltk::image::SvgImage;
use serde::Deserialize;
mod codec_class_toml;
use crate::codec_class_toml::load_all_from_file;
mod flencoder_ui {
...
"##;
    ...
    // for codec in &intermediate_codecs {
    //     ui.codec_choice.add(&codec.name, Shortcut::None, MenuFlag::Normal, |_| {});
    //     ui.encoder_choice.add_choice(&codec.encoder);
    //     ui.container_choice.add_choice(&codec.container);
    //     ui.video_format_choice.add_choice(&codec.video.format);
    //     ui.video_quality_choice.add_choice(&codec.video.quality);
    //     ui.audio_codec_choice.add_choice(&codec.audio.codec);
    //     ui.audio_quality_choice.add_choice(&codec.audio.quality);
    //     if let Some(custom_value) = &codec.custom {
    //         ui.custom_choice.set_value(custom_value);
    //     }
    // }
